...
export default {
  install(Vue) {
     ...
     * @param {String} dataType 指定使用哪种校验方式
    ...
    Vue.prototype.filter_rules = function (item) {
      ...
      item.forEach((element) => {
        ...
        if (element.required) {
          rules.push({
            required: true,
            message: element.message || "该输入项为必填项",
            trigger: "blur",
          });
        }
        /**检测是否需要验证最大字符数 */
        if (element.maxLength) {
          rules.push({
            ...
            message: "最多输入" + element.maxLength + "个字符",
            trigger: "blur",
          });
        }
        /**检测是否需要验证字符区间 */
        if (element.min && element.max) {
          rules.push({
            min: element.min,
            max: element.max,
            message: "字符长度在" + element.min + "至" + element.max + "之间",
            trigger: "blur",
          });
        }
