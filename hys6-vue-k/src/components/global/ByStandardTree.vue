<template>
	<div class="bytree box-sizing d-flex flex-column">
		...
		<div class="mt-20 flex-1 overflow-x-hidden overflow-y-auto">
			<el-tree :indent="0" class="tree-line" :data="data" :props="defaultProps" :default-expanded-keys = "expandedKeys" :filter-node-method="filterNode" node-key="id" ref="tree" @node-click="handleNodeClick">
                ...
                <i class="mr-10 treeNodeIcon" :class="currentId !== data.id ? 'el-icon-folder-opened' : 'el-icon-folder'" v-else></i>
						...
						<el-dropdown-menu slot="dropdown">
							<el-dropdown-item :command="beforeHandleCommand(data,node, 'EDIT')">编辑目录</el-dropdown-item>
							<el-dropdown-item :command="beforeHandleCommand(data,node, 'DEL')">删除目录</el-dropdown-item>
							<el-dropdown-item :command="beforeHandleCommand(data,node, 'PEER')">添加同级目录</el-dropdown-item>
							<el-dropdown-item :command="beforeHandleCommand(data,node, 'SUB')">添加子目录</el-dropdown-item>
							<el-dropdown-item :command="beforeHandleCommand(data,node, 'STANDARD')">添加标准</el-dropdown-item>
						</el-dropdown-menu>
					</el-dropdown>
				</span>
