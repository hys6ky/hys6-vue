  ...
  <div class="standardSource h100 d-flex">
    ...
    <div class="flex-1 px-20 overflow-auto" v-if="isTarget">
      <el-tabs v-model="activeName" @tab-click="handleClick">
        ...
        <el-tab-pane label="历史信息" name="second" v-if="check"></el-tab-pane>
        <el-tab-pane label="规则信息" name="third" v-if="check"></el-tab-pane>
      </el-tabs>
      ...
      <div v-else-if="activeName === 'second'">
        <div class="fr" style="margin-right: 10px ;margin-bottom: 10px">
          <el-button type="primary" @click="historyVersion"> 历史版本 </el-button>
          <el-button style="
            height: 32px;
            padding: 0 14px;
            margin-right: 10px;
            overflow: auto;
          " type="primary" @click.native="ret">返 回</el-button>
        </div>
      ...
      <div v-else>
        <div class="pt-20">
          <FormGroup
            ...
            :tagRule="true"
            @updateValue="ret"
            @addRulesHandler="addRulesHandler"
          >
          </FormGroup>
          <ByTable
    ...
    <div class="flex-1 px-20 overflow-auto" v-else>
      ...
      ></ByQuickSearch>
      <div class="mt-20">
        <ByContainerTitle title="标准列表">
          <div>
            <el-button type="primary" v-debounce @click="addTarget"
              >新建</el-button
            >
            <el-button type="major" @click="batchDel">批量删除</el-button>
            <el-button type="major" v-debounce @click="batchData">
              批量发布
            </el-button>
            <el-button type="major" @click="importDataStandards">
              批量导入
            </el-button>
            <el-button type="major" @click="exportDataStandards">
              批量导出
            </el-button>
          </div>
        </ByContainerTitle>
...
import CompareContent from "@/bizpot/K/standardSource/components/compareContent.vue";
export default {
  name: "standardSource",
  components: {CompareContent, ByTree, FormGroup },
  watch: {
    "formData.data_type": {
      handler(val) {
        let type = "";
  ...
  methods: {
    ...
    setOption(data,prop,dataValue){
      data.map(item =>{
        if (item.prop === prop){
          item.options = dataValue;
        }
      })
    },
    //历史版本
    ...
    handleClick(tab, event) {
      ...
      if(tab.name === 'second'){
        this.$executeRequest
            .execGetByControllerAllMappingName("/K/dbm/normbasic/getHisData",{basic_id : this.basic_id}).then(res =>{
              if (res && res.success){
                res.data.map(item =>{
                  if (item.version  === 0 || item.version === "0"){
                    item.version = "当前版本";
                  }
                  if('1' === item.norm_status ){
                      item.verSionStatus = {
                        text: "已发布",
                        color: "#3BC698",
                      }
                  }else if ((item.version === 0 || item.version === "0"  || item.version === "当前版本")
                      && item.norm_status === "0"){
                    item.verSionStatus = {
                      text: "未发布",
                      color: "#666666",
                  ...
                  this.historyTabelData = res.data;
              } else  {
                this.historyTabelData = [];
              }
        })
      }else if (tab.name === "third"){
        this.rulesFormData = {
          standardName: this.formData.norm_cname,
        }
        this.getDbmNormbasSelect();
        this.getDataQuality();
      }
    },
    getDataQuality(){
      ...
      this.$executeRequest.execGetByControllerAllMappingName("/K/dbm/dataQuality/getDataQuality",params).then(res =>{
        if (res && res.success){
          res.data.listData.map(item =>{
            item.createTime = this.$dateconversion.dateFormat(item.created_date)+" "+this.$dateconversion.hourFormat(item.created_time)
          })
          console.log(res.data.listData)
          this.rulesTableData = res.data.listData;
          this.pagination.total = res.data.totalSize;
        }
      })
    },
    getDbmNormbasSelect(){
      this.$executeRequest.execGetByControllerAllMappingName("/K/dbm/normbasic/getDbmNormbasSelect").then(res =>{
        if(res && res.success){
          this.setOption(this.rulesModelFormItems,"dq_refer_basic_id",res.data)
        }
      })
    ...
    linkTo(row) {
      console.log(row);
      // this.getDbmNormbasicInfoById(row.basic_id);
      this.getHisData(row.basic_id,row.version)
      this.activeName = 'first';
    },
    getHisData(basic_id,version){
      if(version === "当前版本"){
        version = '0';
      }
      let params = {
      ...
      this.$executeRequest
          .execGetByControllerAllMappingName(
            ...
            if(res && res.success){
              ...
              if( !res.data.version || res.data.version === "0" ||res.data.version === 0  ){
                res.data.version = "当前版本";
              }
    ...
    drawRulesConfirm() {
      this.$refs.rulesDrawerForm.transferData().then((data) => {
        if (!data.dq_id){
          this.saveDataQuality(data);
        }else {
          this.upDataQuality(data);
        }
        // console.log(data);
        // this.rulesVisible = false;
      });
    },
    saveDataQuality(data){
      data.basic_id = this.basic_id
      this.$executeRequest
          .execPostByControllerAllMappingName("/K/dbm/dataQuality/saveDataQuality",data).then(res =>{
        if (res && res.success){
    ...
    operateHandler(type, row) {
      if (type === "edit"){
        this.getOneDataQuality(row.dq_id);
        this.rulesVisible = true
      }else if (type === 'del'){
        // alert("删除操作");
        this.deldataQuality(row.dq_id);
      }
      // type === "edit" ? (this.rulesVisible = true) : "";
    },
    deldataQuality(dq_id){
      this.$executeRequest.execPostByControllerAllMappingName("/K/dbm/dataQuality/delQuality?dq_id="+dq_id).then(res =>{
        if(res && res.success){
    ...
    getOneDataQuality(dq_id){
      this.$executeRequest.execGetByControllerAllMappingName("/K/dbm/dataQuality/getOneDataQuality",{dq_id : dq_id}).then(res =>{
        if (res && res.success){
          this.rulesModelFormData = res.data;
        }
      })
    },
    upDataQuality(data){
      this.$executeRequest.execPostByControllerAllMappingName("/K/dbm/dataQuality/upDataQuality",data).then(res =>{
        if(res && res.success){
          this.$Msg.customizTitle("修改成功","success");
          this.getDataQuality();
          this.rulesVisible = false;
    ...
    getDbmNormbasicInfoBySortId(id) {
      ...
      this.$executeRequest
        .execPostByControllerAllMappingNameParams(
          "/K/dbm/normbasic/getDbmNormbasicInfoBySortId",
          params
        )
        .then((res) => {
          if (res && res.success) {
            ...
            console.log(res.data.dbmNormbasicInfos);
    ...
    getDbmNormbasicInfo() {
      ...
      this.$executeRequest
        .execPostByControllerAllMappingNameParams(
          "/K/dbm/normbasic/getDbmNormbasicInfo",
          params
        )
        .then((res) => {
          if (res && res.success) {
