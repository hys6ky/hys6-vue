<template>
  <div class="bytree box-sizing d-flex flex-column">
    ...
    <div class="mt-20 flex-1 overflow-auto" id="treeWrap">
      <el-tree
        :indent="0"
      ...
      >
        ...
        <span
          slot-scope="{ node, data }"
          class="nodetip"
          v-else-if="data.type === 'textConfig'"
        >
          <span :title="data.description" v-if="data.file_id">
            <i class="el-icon-document mr-10 treeNodeIcon"></i>
          ...
          <span :title="data.description" v-else-if="data.file_id == null && data.children.length === 0">
            <i class="mr-5 treeNodeIcon"
               :class="
                  currentId === data.id
                    ? 'el-icon-folder-opened'
                    : 'el-icon-folder'
