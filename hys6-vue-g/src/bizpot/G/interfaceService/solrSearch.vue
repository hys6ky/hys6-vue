<template>
  <div class="solrSearch px-20">
    <div class="mt-10">
      <el-button icon="el-icon-back" class="goBackBtn" @click="$router.go(-1)"
      >返回</el-button
      >
    </div>
    <by-container-title title="Solr查询接口接口说明"  class="my-10"></by-container-title>
<!--    <el-row class="topTitle">-->
<!--      <span class="el-icon-s-operation">Solr查询接口接口说明</span>-->
...
<!--          >outType=stream, dataType=JSON 正常响应事例:</span-->
<!--        >-->
...
<!--        <span class="fontStyle"-->
<!--          >outType=stream, dataType=CSV 正常响应事例:</span-->
<!--        >-->
        <pre style="font-size: 14px">
          ...
          ApiColumnArr,
            ...
            tableData: [{
                ...
                },
                {
                    field: 'column',
                    fieldType: 'String',
                    isRequired: '必填(column=列名=搜索内容,column2=dkldk,column3=jjj;多个列和值键值对以逗号隔开)(输入搜索内容说明,如果是范围查找输入[100 TO 1000]或[* TO 100],或[100 TO *]支持数字类型;',
                    remark: '查询过滤条件',
                },
                {
                    field: 'num',
                    fieldType: 'String',
                    isRequired: '选填(num=1,10;代表从第几条开始返回多少条数据;不填写默认返回前10条)',
                    remark: '显示条数',
                },
                {
                    field: 'dataType',
      ...
      getIpAndPort() {
        this.$executeRequest.execGetByModuleUrl('/interfaceService/serviceuser/getIpAndPort').then(res =>{
          ...
          this.requestAddressForToken =
              ...
              "&columns=zhangsan&column=date=djj,date=hua&dataType=json&outType=stream&asynType=0";
          this.requestAddress = this.ipAndPort +
              "?user_id=1005&user_password=111111&url=" + this.$route.query.url + "&tableName=emp" +
              "&columns=zhangsan&column=date=djj,date=hua&dataType=json&outType=stream&asynType=0";
        })
      },
    }
