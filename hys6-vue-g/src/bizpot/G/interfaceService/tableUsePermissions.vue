<template>
  <div class="tableUsePermissions px-20">
    <div class="mt-10">
      <el-button icon="el-icon-back" class="goBackBtn" @click="$router.go(-1)"
      >返回</el-button
      >
    </div>
    <by-container-title title="表使用权限接口说明" class="my-10"></by-container-title>
<!--    <el-row class="topTitle">-->
<!--      <span class="el-icon-s-operation">表使用权限接口说明</span>-->
        ...
        <pre style="font-size: 14px">
                {
                    ...
                    "message":"Success",
                    "data":{
                        "ORIGINAL_NAME":"eaj05BrResult",
                        "TABLE_BLSYSTEM":"DCL",
                        "SYSREG_NAME":"eaj05BrResult"
                    },
                    "success":true
                }
            </pre
      ...
      getIpAndPort() {
        this.$executeRequest.execGetByModuleUrl('/interfaceService/serviceuser/getIpAndPort').then(res =>{
          ...
          this.requestAddressForToken =
              this.ipAndPort + "?token=abcdef&url=" + this.$route.query.url
