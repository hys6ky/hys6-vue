<template>
  <div class="sqlInterfaceSearch px-20">
    <div class="mt-10">
      <el-button icon="el-icon-back" class="goBackBtn" @click="$router.go(-1)"
      >返回</el-button
      >
    </div>
    <by-container-title title="sql查询接口说明" class="my-10"></by-container-title>
    ...
    return {
      ...
      tableData: [
        ...
        },
        {
          ...
          fieldType: "String",
          isRequired:
            "与参数asynType一起使用(如果asynType为2,则必填轮询返回文件名称)",
          remark: "轮询返回文件名称",
        },
        {
          field: "filepath",
      ...
      this.$executeRequest
        ...
        .then((res) => {
          this.ipAndPort =
            "http://" +
            res.data +
            "/G/interfaceService/serviceuser/impl/" +
          ...
          this.requestAddress =
            this.ipAndPort +
            "?user_id=1005&user_password=111111&url=" +
            this.$route.query.url +
            "&sql=select a from b&dataType=json&outType=stream&asynType=0";
        });
    },
