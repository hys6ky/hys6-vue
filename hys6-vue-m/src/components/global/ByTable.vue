  ...
  <div class="byTable">
    ...
    >
      <template v-for="(i, j) in columnArr">
        ...
        >
          <template slot-scope="scope">
            <span>{{ scope.row[i.prop] | filterNum }}</span>
          </template>
        </el-table-column>
        <el-table-column
...
export default {
  ...
  filters: {
    ...
    filterNum: function (value) {
      return value && value.toString().length > 0 ? value.toString() : "0";
    },
  },
  methods: {
    dateFormat,
