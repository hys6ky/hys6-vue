...
import {execUploadFileByModuleUrl} from "@/utils/executeRequest";
...
export default {
  ...
  methods: {
    ...
    add(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.$executeRequest.execPostByModuleUrl("/dataCollectionM/saveDataSource", params).then(response => {
    ...
    upload(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.$executeRequest.execUploadFileByModuleUrl("/dataCollectionM/uploadFile", param).then(res => {
            this.$emit("addEvent");
    ...
    excelUpload(upload) {
      ...
      this.$refs['formExcelImport'].validate(valid => {
        if (valid) {
          ...
          this.$executeRequest.execUploadFileByModuleUrl("/dataCollectionM/importExcel/importDatabaseByExcel", param).then(res => {
            if (res && res.success) {
    ...
    //下载Excel模板
    downloadExcel() {
      this.$executeRequest.execDownloadFileByModuleUrl("/dataCollectionM/importExcel/downloadExcel").then(res => {
