      ...
      <el-col :span="18">
          ...
          <el-row :gutter="20">
            <el-col :span="12">
              <el-form-item label="列分割符" prop="database_separatorr">
                <el-input
                  v-model="form.database_separatorr"
                  placeholder="请输入列分隔符"
                  style="width: 300px"
                >
                </el-input>
          ...
          <el-row :gutter="20">
            ...
            <el-col :span="12">
              <el-form-item label="是否压缩" prop="is_compress">
                <el-radio-group v-model="form.is_compress">
                  <el-radio
                    v-for="(item, index) in isFlag"
                    :key="index"
                    :label="item.code"
          ...
          <el-row :gutter="20">
            <el-col :span="12">
              <el-form-item label="文件名是否大写" prop="is_upper">
                <el-radio-group v-model="form.is_upper">
                  <el-radio
                    v-for="(item, index) in isFlag"
                    :key="index"
                    :label="item.code"
            ...
            <el-col :span="12">
              <el-form-item label="是否标识文件" prop="is_flag">
                <el-radio-group v-model="form.is_flag">
                  <el-radio
                    v-for="(item, index) in isFlag"
                    :key="index"
                    :label="item.code"
            ...
            <el-col :span="12">
              <el-form-item label="是否发布" prop="is_release" hidden>
                <el-input v-model="form.is_release" value="0"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="20">
        ...
        <el-row>
          ...
          <el-button
            ...
            size="medium"
            class="rightbtn"
            @click="formaterSql(form.sql_table)"
            >格式化sql
          </el-button>
        </el-row>
      </el-col>
...
export default {
  ...
  methods: {
    ...
    getWebSQLTreeData() {
      this.$executeRequest
        .execGetByPostModuleUrl("/dataDistribution/getWebSQLTreeData")
        .then((res) => {
          if (res.success) {
    ...
    handleNodeClick(data) {
      ...
      if (data.file_id && data.file_id.length > 0) {
        // this.form.sql_table = data.hyren_name;
      }
    ...
    changesql() {
      ...
      this.$refs.sqleditorOne.setmVal(sql);
      this.form.sql_table = sql;
      this.iftablecolumn = false;
    },
    changeTextarea(val) {
      this.$set(this.form, "sql_table", val);
    },
    formaterSql(val) {
      this.$refs.sqleditorOne.sqlFormatter();
    },
    saveDistributeData(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$executeRequest
            .execGetByPostModuleUrl(
              "/dataDistribution/saveDistributeData",
              this.form
            )
            .then((res) => {
              if (res && res.success) {
