...
export default {
  ...
  created() {
     this.dd_ids = this.$route.query.dd_ids
  ...
  methods: {
    getIsReleaseData(dd_ids1, page1, size1) {
      let params = {};
      params["dd_ids"] = dd_ids1;
      params["currPage"] = page1;
      params["pageSize"] = size1;
      this.$executeRequest
        .execPostByModuleUrl("/dataDistribution/getIsReleaseData", params)
        .then((res) => {
          this.ruleForm.startuptableData = res.data.releaseInfo;
        ...
        this.$confirm("确认发布吗?", "提示", {
          ...
          this.$refs[formName].validate((valid) => {
            if (valid) {
              ...
              this.$executeRequest
                .execPostByModuleUrl(
                  "/dataDistribution/saveDistributeDataJobRelation",distributeJobBean
                )
                .then((res) => {
                    if (res && res.success) {
    ...
    getJobMsg() {
      this.$executeRequest.execPostByMenuUrl("/getJobMsg", {dd_ids: this.dd_ids}).then((res) => {
          if (res && res.success) {
            ...
            for (let i = 0; i < res.data.length; i++) {
              if (res.data[i].length == 2) {
                ...
                this.ruleForm.startuptableData[i].dd_id = res.data[i][1].dd_id;
              } else if (res.data[i].length == 3) {
                this.ruleForm.startuptableData[i] = res.data[i][0];
                this.ruleForm.startuptableData[i].dd_id = res.data[i][2].dd_id;
