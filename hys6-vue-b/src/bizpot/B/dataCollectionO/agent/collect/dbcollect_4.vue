    ...
    <div id="steps6">
        ...
        <el-form :model="ruleForm" status-icon ref="ruleForm" :label-position="labelPosition" label-width="150px"
            ...
            <div class="startupform">
                <el-row type="flex">
                    <el-col :span="8">
                        <el-form-item label="作业调度工程编号" prop="etl_sys_id" :rules="filter_rules([{required: true}])">
                            <el-col :span="16">
                                <el-select v-model="ruleForm.etl_sys_id" filterable clearable placeholder="请选择"
                                           @change="getEtlSubSysFun">
                    ...
                    <el-col :span="8">
                        <el-form-item label="作业调度任务编号" prop="sub_sys_id" :rules="filter_rules([{required: true}])">
                            <el-col :span="16">
                        ...
                        <el-form-item label="上游作业" prop="Upstream_operation">
                            <el-col :span="16">
                                <el-select style="width:100%" filterable v-model="ruleForm.Upstream_operation" multiple
                                    ...
                                    <el-option v-for="(itx) in preJobName" :key="itx.code" :label="itx.value" :value="itx.code"></el-option>
                                </el-select>
                            </el-col>
                        </el-form-item>
                    </el-col>
                ...
                <el-row type="flex" style="margin-left: -40px;">
                    <el-col :span="12">
                        <el-form-item label="作业程序目录" prop="pro_dic" :rules="rule.default" class="ml_lable">
                            <el-col :span="24">
                                <el-input v-model="ruleForm.pro_dic" placeholder="作业程序目录" size="medium"></el-input>
                            </el-col>
                        </el-form-item>
                    </el-col>
                    <el-col :span="12" style="margin-left: -20px;">
                        <el-form-item label="采集作业日志目录" prop="log_dic" :rules="rule.default" class="ml_lable">
                            <el-col :span="24">
                                <el-input v-model="ruleForm.log_dic" placeholder="采集作业日志目录" size="medium"></el-input>
                            </el-col>
                        </el-form-item>
                    </el-col>
                </el-row>
            ...
            <div class="steps4">
                <el-table :header-cell-style="{background:'#e6e0e0'}" ref="filterTable" stripe
                    ...
                    <el-table-column label="作业优先级" align="center" show-overflow-tooltip width="140px">
                        <template slot="header">
                            <el-tooltip class="item" effect="light" placement="right">
                                <div slot="content">作业优先级设置的数字越大越优先执行</div>
                                <i class="el-icon-question" aria-hidden="true">作业优先级</i>
                            </el-tooltip>
                        </template>
                        <template slot-scope="scope">
                        ...
                        <template slot-scope="scope">
                            <el-form-item :prop="'startuptableData.'+scope.$index+'.pre_etl_job'" :rules="rule.default"
                                ...
                                <el-select style="width:100%" filterable v-model="scope.row.pre_etl_job" multiple
                                    ...
                                    <el-option v-for="item in preJobName" :key="item.code" :label="item.value" :value="item.code"></el-option>
                                </el-select>
                            </el-form-item>
                        </template>
                    </el-table-column>
    ...
    export default {
        ...
        data() {
            return {
                ...
                ProjectnumData: [], //作业调度工程编号数据
                WorknumData: [], //作业调度任务编号数据
                worknum_radio: "",
                Pro_Type: [], //作业程序类型
                preJobName: [],//上游作业名称
                currBatchDate: [
        ...
        methods: {
            ...
            next(formName) {
                this.$refs[formName].validate(valid => {
                    if (valid) {
                        ...
                        // params["etl_sys_cd"] = this.ruleForm.etl_sys_cd;
                        // params["pro_dic"] = this.ruleForm.pro_dic;
                        // params["log_dic"] = this.ruleForm.log_dic;
                        // params["sub_sys_cd"] = this.ruleForm.sub_sys_cd;
            ...
            // 获取工程信息
            getEtlSysDataFun() {
                this.$executeRequest.execPostByModuleUrl("/dataCollectionO/agent/startwayconf/getEtlSysData").then(res => {
                  if (res && res.success) {
                    this.ProjectnumData = res.data;
                    if (this.ruleForm.etl_sys_id) {
                      this.getEtlSubSysFun();
                    }
                  }
                });
            },
            ...
            getAgentPathFun() {
                this.$executeRequest.execGetByModuleUrl("/dataCollectionO/agent/startwayconf/getAgentPath", {
                    ...
                    const logDic = res.data.log_dic;
                    this.ruleForm.log_dic = logDic.substr(0, logDic.lastIndexOf("/") + 1);
                    ...
                    this.etlparaseparator = res.data.etlparaseparator;
                    if (this.ruleForm.etl_sys_id) {
            ...
            getPreJobName() {
                ...
                if (
                    ...
                    this.$Msg.customizTitle("工程编号未选择", 'warning')
                }
            },
            projNumCloseFun() {
                this.projnum_radio = {}
