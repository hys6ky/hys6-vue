    ...
    <div>
        ...
        <el-form ref="ruleForm" :model="ruleForm" class="steps5">
            <el-table :header-cell-style="{background:'#e6e0e0'}" ref="filterTable" stripe
                ...
                <el-table-column label="表中文名" width="240" align="center" :show-overflow-tooltip="true">
                    <template slot-scope="scope">
                        <el-form-item :prop="'ex_destinationData.'+scope.$index+'.table_ch_name'" :rules="rule.default">
                            ...
                            <el-input size="medium" v-model="scope.row.table_ch_name" disabled></el-input>
                        </el-form-item>
                    </template>
                </el-table-column>
                <el-table-column label="是否使用前缀" align="center" width="150" show-overflow-tooltip>
                    <template slot="header">
                        <el-checkbox @change="allPrefix(Allis_prefix)" v-model="Allis_prefix" disabled>
                            <span class="allclickColor">是否使用前缀</span>
                        </el-checkbox>
                    </template>
                    <template slot-scope="scope">
                        <el-checkbox type="selection" :true-label="IsFlag.Shi" :false-label="IsFlag.Fou" disabled
                ...
                <el-table-column label="是否拉链存储" align="center" width="150" v-if="false">
                    <template slot="header">
                ...
                <el-table-column label="是否计算MD5" align="center" width="170" v-if="false">
                    <template slot="header">
                        ...
                        <el-popover placement="right" width="120" height="50" v-model="visible">
                            <el-select v-if="Allis_zippercheck" placeholder="存储方式" v-model="allstoragetype"
                                ...
                                <el-option v-for="(item,index) in zipper22" :key="index" :label="item.value"
                                           :value="item.code" disabled></el-option>
                            </el-select>
                    ...
                    <template slot-scope="scope">
                        <div v-if="scope.row.data_extract_typ!=='1'">
                            <el-form-item :prop="'ex_destinationData.'+scope.$index+'.storage_type'"
                                ...
                                <el-select  placeholder="存储方式"  v-model="scope.row.storage_type" size="medium" disabled>
                                    <el-option v-for="(item,index) in zipper22" :key="index" :label="item.value"
                                               :value="item.code"></el-option>
                                </el-select>
                            </el-form-item>
                ...
                <el-table-column label=" 数据保留天数" align="center" width="165" v-if="false">
                    <template slot="header">
        ...
        <el-row>
            ...
            <el-col :span="12">
                 <!-- <el-button type="primary" size="medium" class='rightbtn' @click="next('ruleForm')">保存</el-button> -->
                 <!-- <el-button type="success" size="medium" class='rightbtn' @click="startButtonFun()">立即启动</el-button> -->
                 <el-button type="success" size="medium" class='rightbtn' @click="openCdcRunManagerDialog(dbid)">运行管理</el-button>
        ...
        <el-dialog title="运行管理" :visible.sync="dialogCdcRunManage" width="40%" class alltable>
          ...
          <el-form :model="cdcRunManagerData">
            <el-form-item label="采集任务" prop="collectTaskList" >
              ...
              <el-table :data="cdcRunManagerData.collectTaskList" empty-text="任务未启动过">
            ...
            <el-form-item label="同步任务" prop="syncTaskList">
              <el-table :data="cdcRunManagerData.syncTaskList" empty-text="任务未启动过">
                ...
                <el-row>
                    <el-col :span="12">
                        <el-form-item label="落地表名:" prop="hyren_name"
                            ...
                            <el-input placeholder="表名" v-model="digForm.hyren_name" size="mini" disabled>
                ...
                <el-table-column width="55" align="center" prop="selectionState">
                    <template slot="header" slot-scope="scope">
                        <el-checkbox @change="Allis_destinationFun(destinationData,Allis_destination)"
                                     v-model="Allis_destination" :checked="Allis_destination" disabled></el-checkbox>
                    </template>
                    <template slot-scope="scope">
                        <el-checkbox :checked="scope.row.usedflag" v-model="scope.row.usedflag"
                                     @change="destination_evercheck(scope.row.usedflag,scope.row.dsl_name)" disabled></el-checkbox>
                    </template>
                </el-table-column>
                <el-table-column label="序号" align="center" width="70">
                    <template slot-scope="scope">
                ...
                <el-table-column :label="item.value" :property="item.id" v-for="item in selectedColumnList"
                    ...
                    <template slot-scope="scope">
                        ...
                        <el-input v-else-if="scope.column.property==='column_ch_name'"
                                  ...
                                  placeholder="列中文名" size="medium" disabled></el-input>
                        ...
                        <el-autocomplete class="fetch" v-else-if="scope.column.property==='column_tar_type'"
                                         ...
                                         @clear="clearSelect" disabled></el-autocomplete>
                        <div v-else>
                            <el-checkbox v-model="scope.row[scope.column.property]" disabled></el-checkbox>
                        </div>
                    </template>
                </el-table-column>
    ...
    export default {
        ...
        beforeCreate() {
            ...
            this.$Code.getCodeItems(params2).then(res => {
                ...
                // zipper = [this.storegaType.QuanLiang, this.storegaType.LiShiLaLian]
                this.codeMethods(params2,res.data);
            });
            // this.$Code.getCategoryItems(params2).then(res => {
            //     let arrdata = res.data ? res.data : []
            //     if (arrdata.length > 0) {
            //         arrdata.forEach(item => {
            //             if (item.code==  this.storegaType.LiShiLaLian ||item.code==this.storegaType.TiHuan||item.code==this.storegaType.UpSet) {
        ...
        methods: {
          codeMethods(params,storegaType){
            this.$Code.getCategoryItems(params).then(res => {
              console.log(res,"res")
              let arrdata = res.data ? res.data : []
              if (arrdata.length > 0) {
                arrdata.forEach(item => {
                  if (item.code==  storegaType.LiShiLaLian ||item.code==storegaType.TiHuan||item.code==storegaType.UpSet) {
                    this.zipper22.push(item)
                    this.notzipper22.push(item)
          ...
          //改变配置状态，如果是实时采集，则点击保存会触发
          setDataBaseFinish(){
            this.$executeRequest.execGetByModuleUrl("/dataCollectionO/agent/stodestconf/saveDatabaseFinish", {
              ...
              if (res.success) {
                ...
                this.$Msg.customizTitle('任务保存成功', 'success')
                this.$router.push({
            ...
            sendSubmit() {
                this.$executeRequest.execGetByModuleUrl("/dataCollectionO/agentList/sendCollectDatabase", {
                    colSetId: this.dbid,
            ...
            nextLinkfun() {
                ...
                if (this.$route.query.edit == "yes") {
                  ...
                  this.setDataBaseFinish(this.dbid);
            },
            pre() {
                let data = {};
                if (this.$route.query.edit == "yes") {
            ...
            AllSettingDestinationFun() {
                ...
                this.$executeRequest.execGetByModuleUrl("/dataCollectionO/agent/stodestconf/getStorageData").then(res => {
                ...
                if (this.Alldestinationchoose.length > 0) {
                    ...
                    for (let i = 0; i < this.ruleForm.ex_destinationData.length; i++) {
                        this.dslIdString.push({
                            ...
                            hyren_name: this.ruleForm.ex_destinationData[i].is_prefix == this.IsFlag.Shi ? this.datasource_number + '_' + this.classify_num + '_' + this.ruleForm.ex_destinationData[i].table_name : this.ruleForm.ex_destinationData[i].table_name,
