  ...
  <div class="singlesearch" id="singleTable">
    ...
    margin-right: 10px;" @keyup.enter.native="searchEnterFun"/>
          ...
          <div class="singleTableinner">
            <el-table ref="filterTable" v-if="tableHeight" :height="tableHeight" stripe
              ...
              <el-table-column width="55" align="center" prop="selectionState">
                <template slot="header" slot-scope="scope">
                  <el-checkbox @change="Allis_selectionStateFun(tableData,Allis_selectionState)"
                               ...
                               :checked="Allis_selectionState" disabled></el-checkbox>
                </template>
                <template slot-scope="scope">
                  <el-checkbox :checked="scope.row.selectionState" v-model="scope.row.selectionState" disabled
              ...
              <el-table-column prop="table_ch_name" label="表中文名" align="center"
                ...
                <template slot-scope="scope">
                  <el-input v-model="scope.row.table_ch_name" placeholder="中文名"
                            @change="changetablechnameFun(scope.row)" disabled></el-input>
                </template>
              </el-table-column>
              <el-table-column prop="is_parallel" label=" 是否并行抽取" align="center" v-if="false" width="130">
                <template slot-scope="scope">
              ...
              <el-table-column prop="sqlFiltering" label="SQL过滤" align="center" width="110" v-if="false">
                <template slot-scope="scope">
              ...
              <el-table-column prop="selectCol" label="选择列" align="center" width="120px" v-if=false>
                <template slot-scope="scope">
                  <el-button size="mini" @click="selectCol(scope.$index, scope.row)" type="info" style="margin:auto">选择列
                  </el-button>
                </template>
              </el-table-column>
              <el-table-column label="实时采集" align="center" v-if="false">
                <template slot-scope="scope">
      ...
      <el-tab-pane label="使用SQL采集数据" name="second" v-show="false">
    ...
    <el-dialog title="选择列" :visible.sync="dialogSelectColumn2" width="80%">
      ...
      <el-table stripe :data="SelectColumnData2" border size="medium" highlight-current-row>
        <el-table-column label="选择列" align="center" width="85px">
          <template slot="header" slot-scope="scope">
        ...
        <el-table-column label="主键定义" align="center" width="95px">
          <template slot="header" slot-scope="scope">
...
import Step from "../step_coll1.vue";
...
export default {
  ...
  methods: {
    getAllTableInfo() {
      ...
      this.$executeRequest.execGetByModuleUrl("/dataCollectionO/agent/isolate/getAllTableInfo", params).then(res => {
    ...
    editzdySQLFun() {
      ...
      this.$executeRequest.execGetByModuleUrl("/dataCollectionO/agent/isolate/getAllSQLs", params0).then(res => {
    ...
    steps_getInitInfo() {
      let params = {};
      params["colSetId"] = this.dbid;
      this.$executeRequest.execGetByModuleUrl("/dataCollectionO/agent/isolate/getInitInfo", params).then(res => {
    ...
    schfilter(val) {
      if (val != "") {
        ...
        this.$executeRequest.execGetByModuleUrl("/dataCollectionO/agent/isolate/getTableInfo", params).then(res => {
          this.isLoading = false;
          if (res.data.length > 0) {
            this.Searchzt = true
            let data = res.data;
            ...
            if (istrue.length == 0) {
              ...
              if (rep_table.length > 0) {
                ...
                if (isparmi.length > 0) {
                  //第一个页面卸数方式是增量存在的
                  let params1 = {};
                  params1["tableNames"] = isparmi.join(","); //勾选表并且卸数方式是增量
                  params1["colSetId"] = this.dbid;
                  params1["tableIds"] =
                      JSON.stringify(tableidArr1) === "{}" ?
                          "" :
                          JSON.stringify(tableidArr1);
                  this.$executeRequest.execGetByModuleUrl("/dataCollectionO/agent/isolate/checkTablePrimary", params1).then(res => {
      ...
      if (twotabledata.length > 0) {
        ...
        params1["tableColumn"] =
            JSON.stringify(tableColumn) === "{}" ?
                ...
                JSON.stringify(tableColumn);
                this.activeSec = true;
        // this.$executeRequest.execGetByModuleUrl("/dataCollectionO/agent/isolate/saveAllSQL", params1).then(res => {
        //   if (res.success) {
        //   } else {
        //     this.isLoading = false;
        //   }
    ...
    saveTableConfFun(tableData) {
      let arrData = [],
          ...
          params = {};
      params["colSetId"] = this.dbid;
      this.$executeRequest.execGetByModuleUrl("/dataCollectionO/agent/isolate/getSQLInfoByColSetId", params).then(res => {
        // 遍历拿到所有勾选的数据
        for (let i = 0; i < tableData.length; i++) {
          if (tableData[i].selectionState == true) {
    ...
    saveTableInfofun(arrData, delJson, tableCycles) {
      ...
      params["colSetId"] = this.dbid;
      this.$executeRequest.execGetByModuleUrl("/dataCollectionO/agent/isolate/getColumnInfoByColSetId", params).then(res => {
    ...
    SqlfiltShowFun(name) {
      ...
      this.$executeRequest.execGetByModuleUrl("/dataCollectionO/agent/isolate/getSingleTableSQL", params).then(res => {
    ...
    is_parallelShowFun(id) {
      ...
      let params = {};
      params["tableId"] = id;
      this.$executeRequest.execGetByModuleUrl("/dataCollectionO/agent/isolate/getPageSQL", params).then(rs => {
        if (rs.success) {
    ...
    testParallelExtractionFun(n) {
      ...
      if (this.ruleForm_ParallelEx.EXtable_sql == undefined) {
        ...
        if (this.ruleForm_ParallelEx.EXtable_sql != '') {
          ...
          this.$executeRequest.execGetByModuleUrl("/dataCollectionO/agent/isolate/testParallelExtraction", params).then(res => {
            this.linkloading = false
            if (res.success) {
    ...
    getTableDataCountFun(type) {
      ...
      params["colSetId"] = this.dbid;
      this.$executeRequest.execGetByModuleUrl("/dataCollectionO/agent/isolate/getTableDataCount", params).then(res => {
    ...
    SelectColumnShowFun(name, id, collectState) {
      ...
      this.$executeRequest.execGetByModuleUrl("/dataCollectionO/agent/isolate/getColumnInfo", params).then(res => {
        if (res.data.length == 0) {
          this.tableloadingInfo = "暂无数据";
        } else {
          this.coltable_name = "";
    ...
    getSqlColumnDataFun(row, sql) {
      ...
      this.$executeRequest.execGetByModuleUrl("/dataCollectionO/agent/isolate/getSqlColumnData", params).then(res => {
        if (res.data.length == 0) {
          this.tableloadingInfo = "暂无数据";
        } else {
          this.disShow = row.collectState == false ? true : false;
    ...
    DelRowFun(index, rows) {
      message.confirmMsg("确定删除吗").then(res => {
        if (rows[index].collectState) {
          ...
          this.$executeRequest.execGetByModuleUrl("/dataCollectionO/agent/isolate/deleteTableSql", {
            database_id: rows[index].database_id,
            table_id: rows[index].table_id,
            table_name: rows[index].table_name
          }).then(res => {
            if (res && res.success) {
              this.$executeRequest.execGetByModuleUrl("/dataCollectionO/agent/isolate/getAllSQLs", rows[index].database_id).then(res => {
              })
    ...
    getTableSetUnloadDataFun(id, type) {
      ...
      params["table_id"] = id;
      if (type == "全量") {
        this.$executeRequest.execGetByModuleUrl("/dataCollectionO/agent/isolate/getTableSetUnloadData", params).then(res => {
