<template>
    <div class="singlesearch" id="single1Table">
    ...
    margin-right: 10px;" @keyup.enter.native="searchEnterFun"/>
                    ...
                    <div class="singleTableinner">
                        <el-table ref="filterTable" v-if="tableHeight" :height="tableHeight" stripe
                            ...
                            <el-table-column label="卸数方式" align="center" width="200">
                                <template slot-scope="scope">
                                    <el-row>
                                        <!-- <el-col :span="19"> -->
                                        <el-radio-group v-model="scope.row.unload_type"
                                                        v-if="scope.row.collectState==false">
                                            <el-radio v-for="(item,index) in xsType" :key="index" :label="item.value"
                                                      :disabled="scope.row.unload_type != item.value"
                                                      @click.native.prevent="XSTypeFun(scope.row,item.value)">
                                                {{item.value}}
                                            </el-radio>
                                        </el-radio-group>
                            ...
                            <el-table-column label="计算MD5" align="center" width="130px">
                                <template slot="header" slot-scope="scope">
                                    <el-checkbox @change="handleCheckAllChange(tableData,isMd5All)" v-model="isMd5All"
                                                 :checked="isMd5All">
                                        <span class="allclickColor">计算MD5</span>
                                    </el-checkbox>
                                </template>
                                <template slot-scope="scope">
                                    <el-checkbox v-model="scope.row.is_md5" v-if="scope.row.collectState==true"
                                                 :checked="scope.row.is_md5" @change="md50Fun(scope.row)"></el-checkbox>
                                    <el-checkbox v-model="scope.row.is_md5" v-else disabled :checked="scope.row.is_md5"
                                                 @change="md50Fun(scope.row)"></el-checkbox>
                                </template>
                            </el-table-column>
                            <el-table-column prop="is_parallel" label=" 是否并行抽取" align="center" width="130px">
                                <template slot-scope="scope">
                            ...
                            <el-table-column prop="sqlFiltering" label="SQL过滤" align="center" width="110px">
                                <template slot-scope="scope">
                                    <el-button size="mini" disabled v-if="scope.row.unload_type=='增量'" type="success">
                                        定义过滤
                                    </el-button>
                            ...
                            <el-table-column prop="selectCol" label="选择列" align="center" width="110px">
                                <template slot-scope="scope">
                                    <el-button size="mini" @click="selectCol(scope.$index, scope.row)" type="info">选择列
                                    </el-button>
                                </template>
                            </el-table-column>
                        </el-table>
                ...
                <el-form ref="ruleForm" :model="ruleForm" class="steps2">
                    <el-table stripe v-if="tableHeight" :height="tableHeight"
                        ...
                        <el-table-column label="卸数方式" align="center">
                            <template slot-scope="scope">
                                <el-row class="twopageradior">
                                    <!-- <el-col :span="19"> -->
                                    <el-form-item :prop="'sqlExtractData.'+scope.$index+'.unload_type'"
                                                  :rules="rule.selected">
                                        <el-radio-group v-model="scope.row.unload_type"
                                                        v-if="scope.row.collectState==false">
                                            <el-radio v-for="(item,index) in xsType" :key="index"
                                                      :disabled="scope.row.unload_type!=item.value" :label="item.value"
                                                      @click.native.prevent="XSTypeFun2(scope.row,item.value)">
                                                {{item.value}}
                                            </el-radio>
                                        </el-radio-group>
                                        <el-radio-group v-model="scope.row.unload_type" v-else>
                        ...
                        <el-table-column label="计算MD5" align="center" width="120">
                            <template slot="header" slot-scope="scope">
                                <el-checkbox @change="handleCheckAllChange(ruleForm.sqlExtractData,isSqlMd5All)"
                                             v-model="isSqlMd5All" :checked="isSqlMd5All">
                                    <span class="allclickColor">计算MD5</span>
                                </el-checkbox>
                            </template>
                            <template slot-scope="scope">
                                <el-checkbox v-model="scope.row.is_md5" disabled v-if="scope.row.collectState==false"
                                             :checked="scope.row.is_md5"></el-checkbox>
                                <el-checkbox v-model="scope.row.is_md5" v-else
                                             :checked="scope.row.is_md5"></el-checkbox>
                            </template>
                        </el-table-column>
                        <el-table-column prop="is_parallel" label="是否并行抽取" align="center" width="130">
                            <template slot-scope="scope">
                                <el-checkbox disabled v-if="scope.row.unload_type=='增量'||scope.row.collectState==false"
                                             v-model="scope.row.is_parallel" :checked="scope.row.is_parallel"
                                             @change="checkedis_zdyparallelFun(scope.row,scope.$index)"></el-checkbox>
                                <el-checkbox v-else v-model="scope.row.is_parallel" :checked="scope.row.is_parallel"
                                             @change="checkedis_zdyparallelFun(scope.row,scope.$index)"></el-checkbox>
                            </template>
                        </el-table-column>
        ...
        <el-dialog title="选择列" :visible.sync="dialogSelectColumn" width="70%" @close="SelectColumnCloseFun()" top="2%">
            ...
            <el-table stripe :data="SelectColumnData" border size="medium" highlight-current-row>
                ...
                <el-table-column label="拉链字段" align="center">
                    <template slot="header" slot-scope="scope">
                        <el-checkbox @change="zipperChange(SelectColumnData,oneFieldMd5All)" v-model="oneFieldMd5All"
                                     :checked="oneFieldMd5All">
                            <span class="allclickColor">拉链字段</span>
                        </el-checkbox>
                    </template>
                    <template slot-scope="scope">
                        <el-checkbox v-model="scope.row.is_zipper_field" disabled v-if="scope.row.collectState==false"
                                     :true-label="Isflag.Shi" :false-label="Isflag.Fou"></el-checkbox>
                        <el-checkbox v-model="scope.row.is_zipper_field" v-else :true-label="Isflag.Shi"
                                     :false-label="Isflag.Fou"></el-checkbox>
                    </template>
                </el-table-column>
                <el-table-column property="column_name" label="列名" align="center" width="150px"
                                 :show-overflow-tooltip="true"></el-table-column>
                ...
                <el-table-column label="操作" width="120px" align="center">
                    <template slot-scope="scope">
                        ...
                        <el-button size="mini" v-if="disShow==true" disabled style="margin-top: 3px;margin-left: 0px">
                            <i class="el-icon-arrow-down"></i>
                        </el-button>
                        <el-button size="mini" v-else :disabled="scope.$index===(SelectColumnData.length-1)"
                                   @click="moveDown(scope.$index,scope.row,SelectColumnData)" style="margin-top: 3px;margin-left: 0px">
                            <i class="el-icon-arrow-down"></i>
                        </el-button>
                    </template>
                </el-table-column>
        ...
        <el-dialog title :visible.sync="dialog_xsadd" width="50%" class="alltable">
            ...
            <el-form :model="xstypeadd" status-icon ref="xstypeadd" label-width="30%">
                ...
                <el-form-item label="删除SQL" prop="delete">
                    <el-row type="flex" justify="center">
                        <el-col>
                            <el-input v-model="xstypeadd.delete" v-if="collectState" type="textarea" autosize
                                      size="medium" style="width:284px"></el-input>
                            <el-input v-model="xstypeadd.delete" v-else disabled type="textarea" autosize size="medium"
        ...
        <el-dialog title :visible.sync="dialog_xsadd2" width="50%" class="alltable">
            ...
            <el-form :model="xstypeadd2" ref="xstypeadd2" status-icon label-width="30%">
                <el-form-item label="删除SQL" prop="delete">
                    <el-row type="flex" justify="center">
                        <el-col>
                            <el-input v-model="xstypeadd2.delete" v-if="collectState" type="textarea" autosize
                                      size="medium" style="width:284px"></el-input>
                            <el-input v-model="xstypeadd2.delete" v-else disabled type="textarea" autosize size="medium"
                                      style="width:284px"></el-input>
                        </el-col>
                    </el-row>
                </el-form-item>
                <el-form-item label="新增SQL" prop="insert">
                    <el-row type="flex" justify="center">
                        <el-col>
                            <el-input v-model="xstypeadd2.insert" v-if="collectState" type="textarea" autosize
                                      size="medium" style="width:284px"></el-input>
                            <el-input v-model="xstypeadd2.insert" v-else disabled type="textarea" autosize size="medium"
                                      style="width:284px"></el-input>
                        </el-col>
                    </el-row>
                </el-form-item>
                <el-form-item label="更新SQL" prop="update">
                    <el-row type="flex" justify="center">
                        <el-col>
                            <el-input v-model="xstypeadd2.update" type="textarea" v-if="collectState" autosize
                                      size="medium" style="width:284px"></el-input>
                            <el-input v-model="xstypeadd2.update" type="textarea" v-else disabled autosize size="medium"
                                      style="width:284px"></el-input>
                        </el-col>
                    </el-row>
                </el-form-item>
        ...
        <el-dialog title :visible.sync="dialog_xsall" width="50%" class="alltable">
            ...
            <el-form :model="xstypeadd2q" status-icon ref="xstypeadd2q" label-width="30%">
                <el-form-item label="SQL" prop="insert" :rules="rule.default">
                    <el-row type="flex" justify="center">
                        <el-col>
                            <el-input v-model="xstypeadd2q.insert" v-if="collectState" type="textarea" autosize
                                      size="medium" style="width:284px"></el-input>
                            <el-input v-model="xstypeadd2q.insert" v-else disabled type="textarea" autosize
                                      size="medium" style="width:284px"></el-input>
                        </el-col>
                    </el-row>
                </el-form-item>
        ...
        <el-dialog title="选择列" :visible.sync="dialogSelectColumn2" width="70%" top="2%">
            ...
            <el-table stripe :data="SelectColumnData2" border size="medium" highlight-current-row :height="tableHeight">
                ...
                <el-table-column label="拉链字段" align="center" width="120">
                    <template slot="header" slot-scope="scope">
                        <el-checkbox @change="zipperChange(SelectColumnData2,fieldMd5All)" v-model="fieldMd5All"
                                     :checked="fieldMd5All">
                            <span class="allclickColor">拉链字段</span>
                        </el-checkbox>
                    </template>
        ...
        methods: {
            ...
            sqlFun() {
                ...
                if (twotabledata.length > 0) {
                    if (this.zdycallTable.length > 0) {
                        for (let i = 0; i < twotabledata.length; i++) {
                            for (let j = 0; j < this.zdycallTable.length; j++) {
                                if (
                                    twotabledata[i].table_name == this.zdycallTable[j].table_name
                                ) {
                                    if (
                                        twotabledata[i].unload_type == this.zdycallTable[j].unload_type
                                    ) {
                    ...
                    let params1 = {
                      "tableInfoArray":twotabledata.length > 0 ? this.$Secret.Encrypt(JSON.stringify(twotabledata)) : "",
                      "colSetId":this.dbid,
                      "tableColumn":  JSON.stringify(tableColumn) === "{}" ? "" : JSON.stringify(tableColumn)
                    };
            ...
            is_parallelShowFun(id) {
                ...
                let params = {};
                params["tableId"] = id;
                this.$executeRequest.execGetByPostModuleUrl("/dataCollectionO/agent/isolate/getPageSQL", params).then(res => {
            ...
            //第一个页面打开卸数方式设置
            XSTypeFun(row, e) {
                // 如果是编辑并且卸数方式未被选中的不运行操作
                if (!row.collectState && row.unload_type != e) {
                    return;
                }
                row.sql = ''
                if (this.firstTableInfo.length > 0) {
            ...
            XSTypeFun2(row, e) {
                ...
                // 编辑时会把原始sql给初始化为空，现在改为新增时才会初始化为空
                if (row.collectState) {
                    row.sql = ''
                }
                this.Xstable_name = row.table_name;
                ...
                if (type == "全量") {
                    ...
                    this.$executeRequest.execGetByPostModuleUrl("/dataCollectionO/agent/isolate/getTableSetUnloadData", params).then(res => {
                        ...
                        if (res.data.unload_type == "2") {
                            ...
                            this.xstypeadd.insert = data.insert;
