...
<script>
    ...
    export default {
        ...
        methods: {
                    ...
                    if (valid) {
                        ...
                        for (let i = 0; i < arrdata.length; i++) {
                            ...
                            if (arrdata[i].disp_type === 'D') {
                                ...
                                jobRelation[arrdata[i].etl_job] = arrdata[i].pre_etl_job_id.join('^');
                            } else {
                                etlJobs.push({
                                    'pro_type': this.ruleForm.pro_type,
                                    'pro_name': this.ruleForm.pro_name,
                        ...
                        this.$executeRequest.execPostByMenuUrl("/agent/startwayconf/saveJobDataToDatabase", params).then(res => {
            ...
            // 获取路径
            getAgentPathFun() {
                this.$executeRequest.execGetByMenuUrl("/agent/startwayconf/getAgentPath", {
                    colSetId: this.dbid
                }).then(res => {
                    if (res && res.success) {
                        const log_dic = res.data.log_dic;
                        this.ruleForm.pro_name = res.data.pro_name
                        this.ruleForm.pro_type = res.data.pro_type
                        this.ruleForm.log_dic = log_dic.substr(0, log_dic.lastIndexOf("/") + 1)
            ...
            // 获取作业列表
            getPreviewJobFun() {
                this.$executeRequest.execGetByMenuUrl("/agent/startwayconf/getPreviewJob", {
                    colSetId: this.dbid
                }).then(res => {
                    if (res.data.length > 0) {
                        this.ruleForm.startuptableData = res.data
