<template>
  <div>
    <el-row class='topTitle'>
      ...
      <router-link :to="{name:'streamCollectTask'}">
    ...
    saveSdmReceiveConfManage() {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          let data = {}
          var sdmReceiveConf = this.form.Sdm_receive_conf;
          sdmReceiveConf.sdm_agent_id = this.$route.query.sdm_agent_id;
          sdmReceiveConf.sdm_partition_name = data.sdm_partition == '1' ? null : data.sdm_partition_name;
          sdmReceiveConf.msgheader = data.msgtype == '1' ? data.msgheader : null;
          //this.form.sdmMessInfo = data.msgtype == '0' ? this.form.sdmMessInfo : [];
          sdmReceiveConf.sdm_bus_pro_cla = data.cus_des_type == '0' ? null : data.sdm_bus_pro_cla;
          sdmReceiveConf.check_cycle = data.fault_alarm_mode == 'none' ? null : data.check_cycle;
          sdmReceiveConf.sdm_email = data.fault_alarm_mode == 'email' ? data.sdm_email : null;
          sdmReceiveConf.snmp_ip = data.fault_alarm_mode == 'snmp' ? data.snmp_ip : null;
            ...
            if (this.$route.query.type == 'create') {
              this.$executeRequest.execPostByModuleUrl('/dataCollectionO/sdmcollecttask/restjieshoutask/saveSdmRestManage',data).then(response => {
                if (response.success == true) {
                  ...
                  this.$router.push({name:'streamCollectTask'})
                }
              });
