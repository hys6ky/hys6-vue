<template>
  <div>
    <el-row>
      <span style="color: #2196f3;font-size: 18px;">消费数据库设置</span>
    ...
    <div class="content">
      <!-- 数据库参数列表 -->
      <div slot="title">
        <span class="dialogtitle el-icon-edit">数据库参数配置:</span>
      </div>
      <el-form label-width="30%" label-position="right" :model="dataBaseTable" ref="dataBaseTable">
        <el-form-item label="数据库设置编号 :" prop="sdm_db_num" :rules="filter_rules([{required: true}])">
          <el-input placeholder="数据库设置编号" v-model="dataBaseTable.sdm_db_num"></el-input>
        </el-form-item>
        ...
        <!-- 数据库表信息输入 -->
        <div>
          <span class="dialogtitle el-icon-edit">数据库表信息:</span>
        </div>
        <el-form-item label="英文表名 :" prop="sdm_tb_name_en" :rules="filter_rules([{required: true}])">
          <el-input v-model="dataBaseTable.sdm_tb_name_en" placeholder="英文表名">
            <template slot="append">
              <el-button type="primary" @click="checkDataBase">查看表</el-button>
            </template>
          </el-input>
        </el-form-item>
        <el-form-item label="中文表名 :" prop="sdm_tb_name_cn" :rules="filter_rules([{required: true}])">
          <el-input v-model="dataBaseTable.sdm_tb_name_cn" placeholder="中文表名"></el-input>
        </el-form-item>
        <el-form-item label="选择业务处理方式 :" prop="db_bus_type">
          <el-select v-model="dataBaseTable.db_bus_type" style="width:350px">
            <el-option value="0" label="none"></el-option>
            <el-option value="1" label="Java"></el-option>
            <el-option value="2" label="JavaScript"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="Java业务处理类 :" prop="db_bus_class" v-if="dataBaseTable.db_bus_type=='1'">
          <el-input placeholder="Java业务处理类" v-model="dataBaseTable.db_bus_class"></el-input>
        ...
        <el-form-item label="JavaScript业务处理程序 :" prop="db_bus_class" v-if="dataBaseTable.db_bus_type=='2'">
          <el-input placeholder="JavaScript业务处理程序" v-model="dataBaseTable.db_bus_class"></el-input>
        ...
        <div>
          <div>
            ...
            <div>
              <el-form-item label-width="0" style="width:100%;margin-left:0">
                <el-table :data="productData" size="medium" border ref="productData" @selection-change="handleSelectionChange" stripe :header-cell-style="{'text-align':'center'}" :cell-style="{'text-align':'center'}">
                  ...
                  <el-table-column label="类型" width="250px" prop="sdm_var_type">
                    <template scope="scope">
        ...
        <el-form-item style="width:100%;margin-left:0" label-width="0">
          <el-form :model="selfConfigObj" ref="selfConfigObj">
            <el-table stripe border size="medium" :cell-style="{'text-align':'center'}" :header-cell-style="{'text-align':'center'}" :data="selfConfigObj.selfConfig">
              ...
              <el-table-column label="字段英文名称" align="center" :show-overflow-tooltip="true">
                <template slot-scope="scope">
                  <el-form-item :prop="'selfConfig.'+scope.$index+'.column_name'" :rules="filter_rules([{required: true}])" style="width:100%">
              ...
              <el-table-column label="字段中文名称" align="center">
                <template slot-scope="scope">
                  <el-form-item :prop="'selfConfig.'+scope.$index+'.field_ch_name'" :rules="filter_rules([{required: true}])" style="width:100%">
              ...
              <el-table-column label="字段类型" prop="column_type">
                <template scope="scope">
                  <el-form-item :prop="'selfConfig.'+scope.$index+'.column_type'" :rules="filter_rules([{required: true}])" style="width:100%">
    ...
    <!-- 查看数据库表英文表名 -->
    <el-dialog title="数据库表信息" :visible.sync="dataBasedialog" width="500px" :before-close="close">
      <el-table stripe border lazy size="medium" :cell-style="{'text-align':'center'}" :header-cell-style="{'text-align':'center'}" :data="dataBaeInfoList" height="600px" v-loading="isLoading" element-loading-text="加载中...">
        ...
        <el-table-column label="选择表" width="110px">
          <template scope="scope">
            <el-button type="primary" size="small" @click="selectTable(scope.row)">选择表</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  mounted() {
    ...
    this.$executeRequest.execGetByPostModuleUrl('/dataCollectionO/sdmdatadistribute/consumeroperation/getProduceParamsMsg',{
      ...
      this.productData.forEach(res => {
        res.sdm_var_type = res.sdm_var_type == '1' ? 'INT' : res.sdm_var_type == '2' ? 'VARCHAR' : res.sdm_var_type == '3' ? 'FLOAT' : res.sdm_var_type == '4' ? 'VARCHAR' : res.sdm_var_type
      })
    });
    ...
    this.$executeRequest.execGetByPostModuleUrl('/dataCollectionO/sdmdatadistribute/consumeroperation/getFiledType',{
      dsl_id: this.$route.query.dsl_id,
    }).then(res => {
      this.inputTypeGroup = res.data
    });
  ...
  methods: {
    ...
    checkDataBase() {
      ...
      this.isLoading = true
      this.$executeRequest.execGetByPostModuleUrl('/dataCollectionO/sdmdatadistribute/consumeroperation/getConnectTable',{
        dsl_id: this.$route.query.dsl_id
        //   database_ip: this.ip,
        //   user_name:this.dataBase_form.ssd_user_name,
        //   database_name:this.dataBase_form.database_name,
        //   database_port: this.prot
      }).then((res) => {
        if (res.success == true) {
          this.dataBaeInfoList = res.data
          this.isLoading = false
        } else {
          this.isLoading = false
        }
    ...
    //选择表
    selectTable(data) {
      this.dataBaseTable.sdm_tb_name_en = data
      this.dataBasedialog = false
    },
    //获取数据库信息
    getTableData() {
    ...
    submit() {
      ...
      for (let key in this.dataBaseTable) {
        resultForm[key] = this.dataBaseTable[key]
      }
      resultForm.dsl_id = this.$route.query.dsl_id;
      // resultForm.sdm_con_to_db = this.dataBaseTable
      resultForm.is_add = this.$route.query.type == 'edit' ? '0' : '1'
      //delete resultForm.sdm_con_db_id
      // delete resultForm.tab_id
      //delete resultForm.user_id
      try {
        this.$refs.dataBaseTable.validate(valid => {
          // if (valid) {
          ...
          this.$refs.selfConfigObj.validate(valid => {
            if (valid) {
              ...
              this.selected.map(res => {
                ...
                newValue.column_type = res.sdm_var_type
                this.selfConfigObj.selfConfig.push(newValue)
              })
              this.selfConfigObj.selfConfig.forEach(res => {
                // res.column_type = res.column_type == 'VARCHAR' ? 'VARCHAR(30)' : res.column_type
                if (res.column_type == 'VARCHAR2') {
                  res.column_type = 'VARCHAR2(30)';
                } else if (res.column_type == 'VARCHAR') {
                  res.column_type = 'VARCHAR(30)';
                } else {
                  res.column_type = res.column_type;
                }
              })
              resultForm.dqTableColumnBeans = JSON.stringify(this.selfConfigObj.selfConfig)
              this.$executeRequest.execPostByModuleUrl('/dataCollectionO/sdmdatadistribute/consumeroperation/saveDataBaseMsg',resultForm).then((res) => {
                if (res.success == true) {
                  ...
                  let result = []
                  this.selfConfigObj.selfConfig.map((res, index) => {
                    if (res.is_custom != '0') {
                      result.push(res)
                    }
                  })
                  this.selfConfigObj.selfConfig = result
                }
              })
            } else {
