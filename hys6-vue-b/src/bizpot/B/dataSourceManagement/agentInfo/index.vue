...
export default {
  ...
  methods: {
    ...
    // 点击查看获取数据采集信息
    dataCollectInfo() {
      this.$executeRequest.execPostByMenuUrl("/searchDataCollectUser").then(res => {
        if (res && res.success) {
    ...
    add(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.$executeRequest.execByControllerMappingName("/saveAgent", this.formAdd).then(response => {
    ...
    updateAgent(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.$executeRequest.execByControllerMappingName("/updateAgent", this.form).then(res => {
            if (res && res.success) {
    ...
    deleteThisData() {
      this.$confirm('确认删除吗?', '提示', {
        ...
        this.$executeRequest.execByControllerMappingName("/deleteAgent", params).then(res => {
          if (res && res.success) {
