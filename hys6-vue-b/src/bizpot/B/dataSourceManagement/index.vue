...
export default {
  ...
  methods: {
    ...
    //树初始化
    initTree() {
      this.$executeRequest.execByControllerMappingName("/searchDataSourceAndAgentCount")
          .then((res) => {
    ...
    //编辑
    editProject(row) {
      this.$executeRequest.execGetByPostModuleUrl("/dataCollectionM/searchDataSourceById", {
    ...
    downloadProject(row) {
      ...
      this.$executeRequest.execDownloadFileByModuleUrl("/dataCollectionM/downloadFile", params).then((res) => {
    ...
    // 封装调用事件
    getIndexData() {
      this.$executeRequest.execByControllerMappingName("/searchDataSourceAndAgentCount").then(res => {
        if (res && res.success) {
          // 获取所有数据
    ...
    saveChangeAgent(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.$executeRequest.execPostByControllerMappingName("/updateAuditSourceRelationDep", params).then(res => {
            if (res && res.success) {
    ...
    departmentInfo() {
      ...
      this.$executeRequest.execByControllerMappingName("/searchDataSourceById", params).then((res) => {
        if (res && res.success) {
    ...
    handleCurrentChange(val) {
      ...
      this.$executeRequest.execByControllerMappingName("/searchSourceRelationDepForPage", params).then(res => {
        if (res && res.success) {
    ...
    handleCurrentChangeList(val) {
      ...
      this.$executeRequest.execByControllerMappingName("/getDataAuditInfoForPage", params).then(res => {
        if (res && res.success) {
          // 时间日期格式化
    ...
    dataAudit(auth_type, auth_type_zh) {
      this.$confirm('确定' + auth_type_zh + ' 是否继续?', '提示', {
        ...
        this.$executeRequest.execByControllerMappingName("/dataAudit", params).then(res => {
          if (res && res.success) {
    ...
    reclaimAuthority() {
      this.$confirm('确定要回收权限, 是否继续?', '提示', {
        ...
        this.$executeRequest.execByControllerMappingName("/deleteAudit", params).then(res => {
          if (res && res.success) {
