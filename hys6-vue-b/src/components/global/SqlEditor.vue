...
import {execPostByUrl} from "@/utils/executeRequest";
...
export default {
  ...
  methods: {
    ...
    getTablename() {
      //获取sql查询的全部表名
      this.$executeRequest.execByUrlFullPathRequest(
        this.$executeRequest.getGateWayPrefix() + "/Q/sqlConsole/websqlquery/getAllTableNameByPlatform"
      ).then(
        (res) => {
          let arr = [];
    ...
    getTableNameWords(execute_sql) {
      ...
      this.$executeRequest.execPostByUrl("/Q/sqlConsole/websqlquery/getTableColumnInfoBySql", param).then((res) => {
          if (res && res.success) {
            let arr = [];
            let arry = [];
            res.data.forEach((item) => {
