    ...
    >
      ...
      >
          ...
          <el-col :span="24">
            ...
            >
              <el-option
                  ...
                  :value="v.value"
              >
                <span style="float: left">{{ v.label ?v.label : v.value }}</span>
                <span style="float: right; color: #8492a6; font-size: 13px">{{ v.label1 }}</span>
              </el-option>
            </el-select>
            <el-cascader
                style="width: 200px"
        ...
        <div class="fr" v-if="!showMore">
            <el-button type="primary" @click="search"  v-if="!showSearch" v-debounce>查询</el-button>
            <el-button type="major" @click="reset"  v-if="!showSearch" v-debounce>重置</el-button>
          <el-button
              v-for="(item, index) in extraButton"
              :key="index"
              type="major"
              v-debounce
              @click="btnClick(item.prop)"
...
export default {
  ...
  methods: {
    ...
    btnClick(prop) {
      this.$emit("btnClick",prop,this.formData);
    },
    onEmit(e, method) {
      if (method) {
        this.$emit(method, e);
