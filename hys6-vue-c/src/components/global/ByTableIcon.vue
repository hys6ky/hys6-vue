      ...
      <template v-for="(i, j) in columnArr">
        ...
        >
          <template slot-scope="scope">
            <el-link
              type="primary"
              @click="$emit('linkTo', i.operation, scope.$index, scope.row)"
        ...
        >
          <template slot-scope="scope">
            <i
              :class="i.class"
              :title="i.label"
              @click="$emit('linkTo', i.operation, i.prop, scope.row)"
            ></i>
          ...
          <template slot-scope="scope">
            <div class="d-flex align-items-center">
              ...
              >
                ...
                <i
                  v-if="operateItem.type === 'icon'"
                  :class="operateItem.class"
                  :title="operateItem.label"
                  @click="
                    operateItemHandler(
                      operateItem.operation,
                      scope.$index,
                      scope.row
                    )
                  "
...
export default {
  ...
  filters: {
    filterText: function (value) {
      return value && (value.length > 0 || !isNaN(value)) ? value : "--";
      // return value && value.length > 0 ? value : "--";
    },
  },
  methods: {
    operateItemHandler(type, index, row) {
      console.log(type, index, row);
      this.$emit("operateItem", type, index, row);
    },
    operateSelect(val) {
      console.log(val);
