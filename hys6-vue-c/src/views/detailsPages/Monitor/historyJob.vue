...
export default {
    ...
    methods: {
        ...
        monitorHistoryJobInfo() {
            let self = this;
            if (self.form.etl_job == '' || self.form.etl_job == undefined) {
                this.$Msg.customizTitle('作业名称不能为空', 'warning');
                return;
            }
            this.$executeRequest.execGetByModuleUrl('/etlmonitor/monitorHistoryJobInfo',self.form).then(res => {
                if (res && res.success) {
        ...
        downLoad(formName) {
            this.$refs[formName].validate(valid => {
                if (valid) {
                    ...
                    this.$executeRequest.execGetByModuleUrl('/etlmonitor/downHistoryJobLog',{
                        etl_sys_cd: sessionStorage.getItem("sys_cd"),
        ...
        // 下载日志方法封装
        downloadFile(val) {
            this.$executeRequest.downloadFile('/etlsys/downloadFile',{
                fileName: val
            }).then(res => {
                if (res && res.success) {
                    this.filename = val;
        ...
        viewData() {
            ...
            this.$executeRequest.execGetByModuleUrl('/etlmonitor/readHistoryJobLogInfo',{
                etl_sys_cd: sessionStorage.getItem("sys_cd"),
                etl_job: object.etl_job,
