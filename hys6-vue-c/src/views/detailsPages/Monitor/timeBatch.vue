    ...
    <div class="historyBatch">
        ...
        <el-row class="pagination">
            <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
                ...
                layout="total, sizes, prev, pager, next, jumper" :total="totalSize" class='locationcenter' />
        </el-row>
        <el-divider></el-divider>
        <div>
        ...
        <el-row class="pagination">
            <el-pagination @size-change="handleSizeJobChange" @current-change="handleCurrentJobChange"
                :current-page="currJobPage" :page-sizes="[10, 50, 100, 200]" :page-size="pageJobSize"
                layout="total, sizes, prev, pager, next, jumper" :total="totalJobSize" class='locationcenter' />
        </el-row>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            getProjectConsumeTimeSum() {
                this.$executeRequest.execGetByModuleUrl('/etlmonitor/getProjectConsumeTimeSum',{
                    ...
                    if (res && res.success) {
                        for (let i = 0; i < res.data.length; i++) {
                            ...
                            res.data[i].taskNum = res.data[i].taskNum;
                            res.data[i].jobNum = res.data[i].jobNum;
            ...
            getTaskConsumeTimeSum() {
              this.$executeRequest.execGetByModuleUrl('/etlmonitor/getTaskConsumeTimeSum',{
                    etl_sys_cd: this.$route.query.etl_sys_cd,
                    curr_bath_date: this.dateValue,
                    ...
                    if (res && res.success) {
                        for (let i = 0; i < res.data.data.length; i++) {
                            ...
                            res.data.data[i].sub_sys_desc = res.data.data[i].sub_sys_desc;
                            res.data.data[i].jobNum = res.data.data[i].jobNum;
            ...
            getJobConsumeTimeSum() {
              this.$executeRequest.execGetByModuleUrl('/etlmonitor/getJobConsumeTimeSum',{
                    etl_sys_cd: this.$route.query.etl_sys_cd,
                    curr_bath_date: this.dateValue,
                    etl_job: this.etl_job_value,
                    etl_job_desc: this.etl_job_desc,
                    currJobPage: this.currJobPage,
                    pageJobSize: this.pageJobSize
                }).then(res => {
                    if (res && res.success) {
                        for (let i = 0; i < res.data.data.length; i++) {
                            res.data.data[i].curr_end_time = this.dateToMilldate(res.data.data[i].curr_end_time);
                            res.data.data[i].curr_st_time = this.dateToMilldate(res.data.data[i].curr_st_time);
                            res.data.data[i].etl_job = res.data.data[i].etl_job;
                            res.data.data[i].etl_job_desc = res.data.data[i].etl_job_desc;
                            res.data.data[i].jobTime = this.formatSeconds(res.data.data[i].jobTime);
                        }
                        this.jobData = res.data.data
                        this.totalJobSize = res.data.totalSize
                    }
                })
            },
            ...
            handleSizeJobChange(val) {
                this.pageJobSize = val
                this.getJobConsumeTimeSum();
            },
            handleCurrentJobChange(val) {
                this.currJobPage = val
                this.getJobConsumeTimeSum();
            },
            formatSeconds(value) {
            ...
            generateExcel() {
                if (this.projectData.length > 0) {
                    //生成excel表格
                  this.$executeRequest.execGetByModuleUrl('/etlmonitor/generateExcel',{
                        ...
                        if (res && res.success) {
                            ...
                            this.$executeRequest.downloadFile('/etlmonitor/downloadFile',param).then(res => {
                                if (res && res.success) {
                                    fileOperations.fileDownload(res.data, file_name)
                                }
                            })
                        }
                    });
