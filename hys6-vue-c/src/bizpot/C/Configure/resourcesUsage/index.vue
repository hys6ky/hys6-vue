...
<div class="px-20 codeManage">
  <div  style="margin-bottom:10px">
    <ByQuickSearch :form-items="formItems" @search="searchBtn" ></ByQuickSearch>
  </div>
...
<!--    <el-divider></el-divider>-->
  ...
  <div>
    <ByTable :table-data="tableData" :column-arr="columArr" :pagination="pagination" @handleMultiple="handleSelectionChange" @sizeChange="handleSizeChange" @currentChange="handleCurrentChange" @operateItem="operateHandler" v-if="tableData"></ByTable>
...
<!--        </el-table-column>-->
<!--        &lt;!&ndash; <el-table-column show-overflow-tooltip prop="etl_sys_cd" label="工程编号" align='center'>-->
...
<!--    </el-row>-->
    <!-- 添加/修改资源模态框 -->
  <by-model :model-title="resourceTitle" :visible.sync="dialogFormVisibleAdd" @close="beforeClosechange">
    <by-model-form :form-data="formAdd" :form-items="fromItem" :form-rules="frolRules" :form-config="issueFromConfig" ref="modelFrom"></by-model-form>
    <div  slot="modalFoot">
...
<!--                </div>-->
<!--            </el-form-item>-->
<!--            <el-form-item label="作业名称" prop="etl_job_id" :rules="rule.selected">-->
<!--                <el-select style="width:217px" v-model="formAdd.etl_job_id" placeholder="作业名称">-->
...
<!--    </el-dialog>-->
    <!-- 获取上传文件 -->
  <by-model :model-title="'导入资源分配数据'" :visible.sync="dialogImportData" @close="importDatacancel">
<!--  <el-dialog title="导入资源分配数据" :visible.sync="dialogImportData" :before-close="importDatacancel">-->
    ...
    <div slot="modalFoot">
      ...
      <el-button @click="importData" :loading="loadingUpload" >保存</el-button>
    </div>
  </by-model>
<!--    </el-dialog>-->
</div>
...
import {columArr, formItems, frolRules, fromItem, issueFromConfig} from "./mock";
...
let arr = [];
export default {
  components: {ByModelForm, ByModel, ByEmpty, ByTable, ByQuickSearch},
    data() {
        return {
          frolRules: {
            ...
            resource_req: [{required: true, message: '该输入项为必填项', trigger: 'blur'},
    ...
    methods: {
        ...
        getTable() {
            ...
            if(undefined != this.$route.query.etl_job) {
                params["etl_job"] = JSON.stringify(JSON.parse(this.$route.query.etl_job));
            ...
            this.$executeRequest.execPostByModuleUrl('etlMage/jobconfig//searchEtlJobResourceRelaByPage',params).then(res => {
      ...
      validateNumber(rule, value, callback) {
        ...
        getJobName() {
            ...
            let obj = {};
            params["etl_sys_id"] = this.sys_id;
            this.$executeRequest.execGetByModuleUrl('etlMage/jobconfig//searchEtlJob',params).then(res => {
      ...
      setOptions(prop,data){
        ...
        getResourceType() {
            ...
            let obj = {};
            params["etl_sys_id"] = this.sys_id;
            this.$executeRequest.execGetByModuleUrl('etlMage/jobconfig//searchEtlResourceType',params).then(res => {
                res.data.forEach((item) => {
                ...
                this.setOptions('resource_type',arr);
      ...
      operateHandler(type, row){
        ...
        searchBtn(val) {
            ...
            params["etl_job"] = val.etl_job;
            params["resource_type"] = val.resource_type;
            params["currPage"] = this.pagination.pageNum;
            params["pageSize"] = this.pagination.pageSize;
            this.$executeRequest.execPostByModuleUrl('etlMage/jobconfig//searchEtlJobResourceRelaByPage',params).then(res => {
                this.tableData = res.data.jobResourceRelation;
        ...
        handleBatchDelete() {
            if (this.multipleSelection.length == 0) {
                ...
                this.$confirm('确认批量删除吗?', '提示', {
                    ...
                    params.append("etl_job_id", JSON.stringify(arr));
                    this.$executeRequest.execUploadFileByModuleUrl('etlMage/jobconfig//batchDeleteEtlJobResourceRela',params).then(res => {
                        if (res && res.success) {
                            this.getTable();
        ...
        handleDelete(row) {
          console.log(row)
            ...
            this.$confirm('确认删除吗?', '提示', {
                ...
                let params = {};
                params["etl_sys_id"] = this.sys_id;
                params["etl_job_id"] = this.formDelete.etl_job_id;
                this.$executeRequest.execGetByModuleUrl('etlMage/jobconfig//deleteEtlJobResourceRela',params).then(res => {
                    if (res && res.success) {
                        this.getTable();
        ...
        saveAdd(formName) {
          this.$refs.modelFrom.transferData().then( data =>{
                    ...
                    params["etl_job_id"] = this.formAdd.etl_job_id;
                    params["resource_type"] = this.formAdd.resource_type;
                    params["resource_req"] = this.formAdd.resource_req;
                    if (this.resourceTitle == '添加资源分配') {
                        this.$executeRequest.execGetByModuleUrl('etlMage/jobconfig//saveEtlJobResourceRela',params).then(res => {
                            if (res && res.success) {
                                this.getTable();
                        ...
                        this.$executeRequest.execGetByModuleUrl('etlMage/jobconfig//updateEtlJobResourceRela',params).then(res => {
                            if (res && res.success) {
                                this.getTable();
