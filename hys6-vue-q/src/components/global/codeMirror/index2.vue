<template>
<div :id="'editor'+data" style="width:100%;height:200px">
...
import {execPostByMenuUrl} from "@/utils/executeRequest";
export default {
    data() {
        return {
            arry: [],
  ...
  methods: {
    ...
    getTablename() {
      //获取sql查询的全部表名
      this.$executeRequest.execGetByMenuUrl("/websqlquery/getAllTableNameByPlatform").then((res) => {
          let arr = [];
    ...
    getTableNameWords(execute_sql) {
      ...
      this.$executeRequest.execPostByMenuUrl("/websqlquery/getTableColumnInfoBySql", param).then((res) => {
          if (res && res.success) {
            let arr = [];
            let arry = [];
            res.data.forEach((item) => {
