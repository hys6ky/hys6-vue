<template>
    <el-row>
<!--        <i class="el-icon-document" style="font-size: 18px;text-align: center;color: #2196f3;"><span>数据补录审批：{{ defau.pro_name }}</span></i>-->
<!--        <el-button size="small" type="text" style="float:right;" @click="goUpper">返回上一级</el-button>-->
        <ByContainerTitle title="数据补录审批">
          <el-button size="small" type="text" style="float:right;" @click="goUpper">返回上一级</el-button>
        </ByContainerTitle>
...
<!--        </el-form>-->
        <ByTable
          :tableData="tableData"
          :columnArr="applyColumnArr"
          :pagination="pagination"
          @operateItem="click_list_show"
...
<!--            </el-table-column>-->
<!--            <el-table-column prop="update_date" label="最后更新时间" :formatter="formatDateTimeCreate" show-overflow-tooltip align='center'>-->
<!--            </el-table-column>-->
...
<!--        </div>-->
        <el-dialog :visible.sync="dialog.visible" @close="dialogClose">
            <el-table :data="dialog.tableData"  tooltip-effect="dark" stripe border >
                <template v-for="(item, index) in dialog.cols">
                    <el-table-column  :key="index" :prop="item" :label="item" show-overflow-tooltip align='center' :render-header="labelHead"></el-table-column>
                </template>
            </el-table>
        </el-dialog>
    </el-row>
...
export default {
  components: {ByTable, ByQuickSearch, ByContainerTitle},
    data() {
        return {
          applyFormItems,
          applyColumnArr,
    ...
    methods: {
        ...
        initIsFlag() {
            ...
            this.$executeRequest.execGetByMenuUrl("/tableApply/getIsFlag").then(res =>{
              // console.log(res)
              if (res && res.data) {
        ...
        refreshTable() {
            ...
            this.$executeRequest.execGetByMenuUrl("/tableApply/query",data).then(res =>{
              if (res && res.data) {
                ...
                if (tableData) {
                  ...
                  if (tableData.list != null) {
                    tableData.list.forEach(val => {
                      if (val.user_name == null){
                        val.user_name = '';
                      }
                      if (val.is_sync == '1'){
                        val.sync = '已同步'
                      }else if (val.is_sync == '0'){
                        val.sync = '未同步'
                      }
                    })
        ...
        refreshDialog(id) {
            ...
            // })
            // 获取临时表数据
            this.$executeRequest.execGetByMenuUrl("/tableApply/queryData",data).then(res =>{
              if (res && res.data) {
                var d = res.data;
                var cols = [];
                for (var key in d[0]) {
            ...
            // apply.queryTempTableData(data).then(res => {
            //     if (res && res.data) {
            //         var d = res.data;
            //         var cols = [];
            //         for (var key in d[0]) {
    ...
    mounted: function () {
        ...
        this.defau.pro_name = this.$route.query.pro_name;
