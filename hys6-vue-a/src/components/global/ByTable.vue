<template>
  <div class="byTable">
    <el-table
        ...
        :select-on-indeterminate="indeterminate"
    ...
    >
      <template v-for="(i, j) in columnArr">
        ...
        <el-table-column v-else-if="i.type === 'operate'" :key="j" :label="i.label" :fixed="i.fixed || 'right'"
                         :width="i.width || 'auto'" :header-align="i.align || 'left'" :align="i.align || 'left'">
          <template slot-scope="scope">
            <div class="d-flex align-items-center">
              <span v-for="(operateItem, operateIndex) in i.operateArr.filter(
                ...
                <el-button v-else size="mini" type="text" @click="operateItemHandler(operateItem.type, scope.row,scope.$index)">
        ...
        >
          <template slot="header" slot-scope="item">
            <el-input
                v-model="i.queryData"
                :placeholder="i.placeholder || '请输入'" clearable
            >
              <el-button slot="append" @click="$emit('tableInput', i.queryData)"
              >搜索</el-button
              >
            </el-input>
          </template>
    ...
    >
      <div
          class="font-14 totalPageTip" v-if="pagination && pagination.total > 0"
      >
