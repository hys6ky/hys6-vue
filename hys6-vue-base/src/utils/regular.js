 ...
 */
const regular = {
    number: { regex: /^[0-9]*$/, error: '只能填写0与正整数', title: '0与整数' },
    numberAll: { regex: /^\d{1,}$|-\d{1,}$/, error: '只能填写数字', title: '正整数,负整数,0' },
    ...
    auth_path: { regex: /^[/]{1}[a-zA-Z]+[/]+[a-zA-Z]+$/, error: '请使用"/"开头,字母结尾', title: '权限路径' },
    ...
    left_right_space: { regex: /^\S+(\s+\S+)*$/, error: '字符串左右两边不允许有空格', title: '左右空格验证' }
}
